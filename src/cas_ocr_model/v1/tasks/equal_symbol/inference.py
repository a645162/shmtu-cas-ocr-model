...
def _ensure_loaded() -> tuple[torch.device, torch.nn.Module]:
    ...
    if _MODEL is None:
        ...
        _MODEL = init_model(model_equal_symbol_type, 2, pretrained=False).to(_DEVICE)
        pth = os.path.join(
            pth_save_dir_path,
            get_pth_name(model_equal_symbol_type, "equal_symbol", "latest"),
        )
...
def predict_equal_symbol(image_path: str) -> int:
...
if __name__ == "__main__":
    predict_equal_symbol("test/equal_chs.png")
    predict_equal_symbol("test/equal_symbol.png")
